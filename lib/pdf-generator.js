...
function generateReportHTML(data) {
  const { target, brands, prompts, models, reportData, modelData, promptData, industry, city } = data;
  const colorMap = {};
  ...
  const reportBody = `
    ...
    <div class="rs"><h3>AI Visibility Action Plan</h3>
      <div class="ss">Eight moves that compound your AI search visibility over the next 30-90 days</div>
      <ul class="tk">
        <li><b>Get cited in the top "best of" listicles${industry ? ` for ${esc(industry)}` : ''}${city ? ` in ${esc(city)}` : ''}.</b> Pitch your inclusion to the writers behind the articles AI engines are already quoting (see <i>Top Competitors</i> for who they're naming). One earned mention in a Top-3 listicle typically lifts AI visibility 5-15% within 30 days.</li>
        <li><b>Strengthen third-party signals on LinkedIn, Reddit, and Quora.</b> Large language models heavily weight these high-trust domains when generating local-business answers. Active company pages, founder profiles, and helpful answers in your niche compound over 60-90 days.</li>
        <li><b>Add LocalBusiness, Service, and FAQPage schema</b> to your homepage, service pages, and blog posts. Structured data lets AI engines quote your hours, service area, pricing, and FAQ answers verbatim — far more cite-worthy than free-form copy.</li>
        <li><b>Submit to Bing Webmaster Tools and turn on IndexNow.</b> ChatGPT Search runs on Bing's index, so Bing-first indexing is the fastest path to ChatGPT citations. Most local service businesses skip this — the ones that don't earn a structural advantage.</li>
        <li><b>Compound your Google Business Profile signals.</b> Weekly posts, a 4.5+ star average, and at least 50 reviews mentioning your specific services. Google AI Overviews lean heavily on GBP signals for local queries; a thin profile is invisible in AI answers.</li>
        <li><b>Publish answer-first content</b>${industry && city ? ` — FAQ pages and "how to choose a ${esc(industry)} provider in ${esc(city)}" guides` : ' — FAQ pages and how-to-choose guides'} written in chunks LLMs can pull verbatim. Short paragraphs, clear H2/H3 headings, exact-match phrasing of the prompts above.</li>
        <li><b>Allow OAI-SearchBot, ClaudeBot, PerplexityBot, and GoogleOther in robots.txt</b> and add an llms.txt manifest pointing to your most citation-worthy pages. Many sites unintentionally block these crawlers — fixing it costs nothing and unlocks AI visibility immediately.</li>
        <li><b>Earn brand mentions in publications LLMs cite</b> — local news, industry trade pubs, and city-specific blogs. Even a single quote in a credible source meaningfully shifts AI visibility within 30-60 days. Thrive's PR team can broker these placements in any vertical.</li>
      </ul>
    </div>
    <div class="meth">Methodology — Data across ${models.length} AI platforms, ${prompts.length} prompts, ${totalConvo.toLocaleString()} conversations. Visibility = appearances / total conversations. Market Share = brand mentions / total brand mentions. Reputation scored 0-100 (50=neutral). Report generated by Thrive Agency on ${new Date().toLocaleDateString()}.</div>`;
...
async function generatePDF(data, filename) {
  ...
  const waitUntil = process.env.PDF_WAIT_UNTIL || 'networkidle0';
  const timeout = Number(process.env.PDF_TIMEOUT_MS || 30000);
...
module.exports = { generatePDF, generateReportHTML, closeBrowser, getBrowser };

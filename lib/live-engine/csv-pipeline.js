...
async function runGroup({ city, industry, companies }, log = console.log) {
  ...
  // 3. Batch-extract brands per (prompt, model) — one OpenAI call per prompt covering all
  //    models, all prompts extracted in parallel
  log(`  [extract] extracting brands from responses...`);
  const tExtract = Date.now();
  const extractionResults = await Promise.all(promptResults.map(async (pr) => {
  ...
  const flatResponses = extractionResults.flat(); // { promptText, modelName, brands }
  log(`  [extract] ${flatResponses.length} (prompt × model) responses parsed in ${((Date.now() - tExtract) / 1000).toFixed(1)}s`);
